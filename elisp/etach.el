...
;;;     Copyright (C) 2000-2002 John M. Rulnick
	...
	(while filelist
	  (let ((file (car filelist)))
		...
		(etach-file-attach file)
	      (message (concat "not attaching \'" file "\': not a regular file")))
	    (setq filelist (cdr filelist)))))
    (if (file-regular-p files-to-attach)
	(etach-file-attach files-to-attach)
      (message (concat "not attaching \'" files-to-attach "\': not a regular file")))))
